...
function getVodList(khtml, sch = false) {
    try {
        ...
        for (let it of listArr) {
            ...
            // ---- 修复开始 ----
            // 优先取 data-src，没有则取 src
            let rawPic = cutStr(it, 'data-src="', '"', '');
            if (!rawPic || rawPic === '图片') {
                rawPic = cutStr(it, 'src="', '"', '');
            }
            let kpic = rawPic && rawPic !== '图片' && rawPic !== 'cutErr' ? rawPic : '';
            // 若图片地址不是完整http链接，则加上代理前缀
            if (kpic && !/^https?:\/\//i.test(kpic)) {
                kpic = picProxy + encodeURIComponent(kpic);
            }
            // ---- 修复结束 ----
